...
if [[ ! -f "${DEFAULT_NIX}" ]]; then
  PKG_DIR="nixpkgs/gemini-cli"
  DEFAULT_NIX="${PKG_DIR}/default.nix"

  if [[ ! -f "${DEFAULT_NIX}" ]]; then
    echo "❌ Error: Cannot find gemini-cli package directory"
    echo "   Please run from repository root or gemini-cli directory"
    exit 1
  fi
fi

echo "🔄 Updating gemini-cli package..."
...
if echo "$BUILD_OUTPUT" | grep -q "got:    sha256-"; then
  ...
  if (cd "${PKG_DIR}" && NIXPKGS_ALLOW_UNFREE=1 nix-build -E 'with import <nixpkgs> {}; callPackage ./default.nix {}' > /dev/null 2>&1); then
    echo "✅ Successfully updated gemini-cli to version $LATEST_VERSION!"
